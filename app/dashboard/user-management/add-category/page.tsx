...
export default function AddCategoryForm() {
  ...
  const onSubmit = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("User Category Created");
    ...
    if (isParent) {
      ...
      if (permissionNode) {
        if (checked) {
          ...
          updatedPermissions = updatedPermissions.filter((p) => !permissionNode.children.includes(p));
        }
      }
    } else {
      const parentNode = permissions.find((node) => node.children.includes(permission));
  ...
  const formatPermissions = (permissionsData: { [key: string]: {parent:string, children: string[]} }): PermissionNode[] => {
            ...
            <div key={`parent-${permissionNode.value}`} className="mb-4">
              <div className="mb-2">
                <ControlledCheckbox
                  ...
                  setValue={(name, checked) => handlePermissionChange(permissionNode.value, checked, true)}
                />
              </div>
              <div className="pl-4 space-y-3">
                {permissionNode.children.map((childPermission) => (
                  <ControlledCheckbox
                    ...
                    watch={watch}
                    setValue={(name, checked) => handlePermissionChange(childPermission, checked)}
                  />
                ))}
              </div>
            </div>
