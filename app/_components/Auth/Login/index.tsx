...
const Login = ({}) => {
    ...
    return (
        <div className="bg-gray-200 p-8 h-screen w-full">
            <form className=" space-y-2 my-4 w-1/2 mx-auto space-y-4 px-4 py-6 bg-white shadow-md rounded-lg" onSubmit={handleSubmit((data) => onSubmit.mutate(data))} noValidate>
                {onSubmit.isPending}
                <ControlledInput
                    type="email"
                    placeholder="Email"
                ...
                <button className="bg-red-500 w-full rounded-md px-6 py-4 text-xl font-medium text-white" type="submit">
                    Login
                </button>

                <div className="!mt-6 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <input type="checkbox" id="rememberMe" {...register("rememberMe")} className="accent-primary h-6 w-6 rounded-md border border-gray-600/50" />
                        <label htmlFor="rememberMe" className="select-none text-xl text-gray-500">
                            Remember me
                        </label>
                    </div>

                    <button onClick={handleForgetPassword} className="text-primary text-blue-500 cursor-pointer text-xl hover:underline">
                        Forgot password?
                    </button>
                </div>
                <Divider
