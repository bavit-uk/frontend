      ...
      const transformedData =
          {
            ...
            userType: "67403b90e189e381d5f1cdc4", // Assuming this is a fixed value
            dob: data.dob,
            address: [
              {
          ...
          if(!transformedData.address[0]?._id){
            delete transformedData.address[0]._id;
          }

      try {
        const response = await client.put(
          `/user/${data._id}`, // Assuming you're updating by user ID
          transformedData,
          {
            headers: {
    ...
    <div className="px-5">
      ...
      >
        ...
        <ControlledSelect
          label="User Category"
          name="userCategory"
          options={userCategory.map(
            (category: { _id: string; role: string }) => ({
              value: category._id,
              label: category.role,
            })
          )}
